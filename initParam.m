...
for i = 1:size(city.Buildings,1)
    for j = 1:size(city.Buildings,2)
        fprintf(file, '%4.2f', city.Buildings(i,j));
        if(j==size(city.Buildings,2))
            fprintf(file, '\n');
        else
            fprintf(file, ', ');
        end
...
switch Param.trafficModel
    ...
    case 'webBrowsing'
        if (exist('traffic/webBrowsing.mat', 'file') ~= 2 || Param.reset)
			Param.trSource = loadWebBrowsingTraffic(Param.schRounds*10^-3, Param.numUsers, Param.seed);
		else
